...
async function transcribeWithWhisper(
  ...
  const audioFileBlob = new Blob([new Uint8Array(audioBuffer)], {
    type: mimeType || "audio/mpeg",
  });
  const audioFileForApi = new File([audioFileBlob], fileName, {
    type: mimeType || "audio/mpeg",
  });
 ...
 * Transcribe an audio file using OpenAI Whisper API, Assembly AI, or GPT-4o
 */
export async function POST(
  ...
  try {
    ...
    const audioBuffer = await getAudioFileBuffer(id);
    if (!audioBuffer) {
      return NextResponse.json(
