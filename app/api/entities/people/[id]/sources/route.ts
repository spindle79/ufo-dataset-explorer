/**
 * API Route: Get source items for a person
 * GET /api/entities/people/[id]/sources
 */
...
export async function GET(
  ...
  try {
    ...
    const sources = await Promise.all(
      (relationships || []).map(async (rel) => {
        if (rel.source_type === "scrape") {
          ...
          // For pdf, audio, video - they're in original_uploads
          const { data } = await supabase
    ...
    return NextResponse.json(
      {
        error: "Failed to fetch person sources",
        message:
