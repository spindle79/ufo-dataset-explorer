...
import { createImageFile } from "@/lib/image-access";
  ...
  try {
    ...
    if (!urlColumn) {
      return NextResponse.json(
        {
          error:
            'CSV file must contain a column named "url", "image_url", "link", or "image_link"',
        },
        { status: 400 }
      );
    }
    ...
    for (const record of records) {
      ...
      try {
        ...
        if (!contentType || !contentType.startsWith("image/")) {
          failedCount++;
          errors.push(`URL ${url} does not point to an image file`);
          continue;
        }

        const arrayBuffer = await response.arrayBuffer();
        const buffer = Buffer.from(arrayBuffer);
        const fileName = url.split("/").pop() || "image_file";
