...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!replaceExisting) {
      ...
      const duplicate = await findDuplicateImageFileByUrl(canonicalUrl, url);
      if (duplicate) {
        return NextResponse.json(
          {
    ...
    let fileName: string = url.split("/").pop() || "image_file";

    // If skipDownload is true, create record without downloading
    if (skipDownload === true) {
      const imageFile = await createImageFileFromUrl({
        fileName,
        originalUrl: url,
