...
import { createVideoFile } from "@/lib/video-access";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const urlColumn = Object.keys(records[0]).find(
      (key) =>
        key.toLowerCase() === "url" ||
        key.toLowerCase() === "video_url" ||
        key.toLowerCase() === "link" ||
        key.toLowerCase() === "video_link"
    );
    ...
    if (!urlColumn) {
      return NextResponse.json(
        {
          error:
            'CSV file must contain a column named "url", "video_url", "link", or "video_link"',
        },
        { status: 400 }
      );
    }
    ...
    for (const record of records) {
      ...
      try {
        ...
        if (!contentType || !contentType.startsWith("video/")) {
          failedCount++;
          errors.push(`URL ${url} does not point to a video file`);
          continue;
        }

        const arrayBuffer = await response.arrayBuffer();
        const buffer = Buffer.from(arrayBuffer);
        const fileName = url.split("/").pop() || "video_file";
