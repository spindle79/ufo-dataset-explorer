...
/**
 * POST /api/video/[id]/generate-description
 * Generate a description for a video file using OpenAI gpt-5-nano
 */
  ...
  try {
    ...
    if (!videoFile.currentTranscriptId) {
      return NextResponse.json(
        {
          error:
            "No transcript available for this video file. Please transcribe the video first.",
        },
        { status: 400 }
      );
    }
    ...
    // Update the video file with the generated description
    const updated = await updateVideoFile(id, { description });

    if (!updated) {
      return NextResponse.json(
        { error: "Failed to update video file" },
        { status: 500 }
      );
    }
