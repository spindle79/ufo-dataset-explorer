...
export async function POST(
  ...
  try {
    ...
    if (updateError || !updatedRecord) {
      return NextResponse.json(
        { error: updateError?.message || "Failed to include PDF file" },
        { status: 500 }
      );
    }
    ...
    console.error("Error including PDF file:", error);
    return NextResponse.json(
      {
        error:
          error instanceof Error ? error.message : "Failed to include PDF file",
      },
      { status: 500 }
    );
  }
