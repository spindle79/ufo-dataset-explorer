...
import { Loader2, ExternalLink, FileText, Music, Video, File, FolderKanban, Info } from "lucide-react";
...
export default function ProgramDetailPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const programResponse = await fetch(`/api/entities/programs/${id}`);
        ...
        const sourcesResponse = await fetch(`/api/entities/programs/${id}/sources`);
            ...
            <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg p-6">
              {program.description && (
                <div className="mb-4">
                  ...
                  <p className="text-gray-900 dark:text-white">{program.description}</p>
                </div>
              )}

              {program.aliases && program.aliases.length > 0 && (
                <div className="mb-4">
                  <h2 className="text-sm font-semibold text-gray-600 dark:text-gray-400 mb-2">
                    Aliases
                  </h2>
                  <div className="flex flex-wrap gap-2">
                    {program.aliases.map((alias, idx) => (
                      <span
                        key={idx}
            ...
            <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg p-6">
              ...
              {sources.length === 0 ? (
                <p className="text-gray-500 dark:text-gray-400">
                  No source items found. This program has not been extracted from any documents yet.
                </p>
              ) : (
