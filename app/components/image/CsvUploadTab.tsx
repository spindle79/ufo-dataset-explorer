...
export default function CsvUploadTab({ onUploadSuccess }: CsvUploadTabProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch("/api/image/csv", {
        method: "POST",
        body: formData,
      });
    ...
    <div>
      <div className="mb-6 p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-md">
        ...
        <p className="text-sm text-blue-800 dark:text-blue-300">
          Your CSV file should have a column with URLs. The first row should be
          headers. Expected format:{" "}
          <code className="bg-blue-100 dark:bg-blue-800 px-1 rounded">url</code>{" "}
          or{" "}
